...
__REDUCTIONS__ = {
    "sum": torch.sum,
    "mean": torch.mean,
    "iou": lambda t: torch.mean(
        1.0 - (t / t[0].numel())
    ),  # NOTE: this reduction is only correct for batch size = 1?
}
...
class Weighted(torch.nn.ModuleDict):
    def __init__(
        ...
        **kwargs: typing.Mapping[str, typing.Any],
        ...
        if not len(objectives):
            log.warning(
                "A weighted combination of objectives is being used for supervising the model, but no losses have been assigned."
            )
        ...
        for i, key in enumerate(kwargs):
            if key not in objectives:
                log.warning(
                    f"Skipping objective [red strike bold]`{key}`[/] as it is not found in the configuration :exclamation:"
                )
                continue
            try:
                self.add_module(
                    key, hyu.instantiate(objectives[key])
            ...
            if (
                extra_params := set(objective_kwargs.keys())
                - set(sig_params)
            ...
            for j, params in enumerate(objective_kwargs):
                ...
                if not (reduction := toolz.get(C._REDUCTION_, params, default=None)):
                    log.warning(
                        f":warning: Objective [yellow bold]`{key}`[/] has no assigned reduction, automatically reverting to mean reduction."
                    )
                    reduction = "mean"
                self.execs.append(
                    (
    ...
    def forward(self, tensors: typing.Dict[str, torch.Tensor]) -> torch.Tensor:
        for i, (key, out, weight, reduction, kwargs) in enumerate(self.execs):
            error = self[key](
                **toolz.valmap(lambda v: tensors[v] if v is not None else v, kwargs)
            )
            ...
            if i == 0:
                tensors[f"{C._MOAI_LOSSES_}.total"] = tensors[
                    f"{C._MOAI_LOSSES_}.weighted.{out}"
                ]
            else:
                tensors[f"{C._MOAI_LOSSES_}.total"] += tensors[
                    f"{C._MOAI_LOSSES_}.weighted.{out}"
