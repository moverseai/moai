...
@dataclasses.dataclass(
    unsafe_hash=True
)  # NOTE: needed for hashing https://stackoverflow.com/questions/60289768/error-unhashable-type-dict-with-dataclass
...
class NnaryOperationTensors(torch.nn.Module):
    ...
    def forward(self, td, tmp) -> None:  # torch.Tensor:
        tmp[f"result{self.index}"] = self.op(
            [toolz.get_in(k.split("."), tmp) for k in self.keys], dim=self.dim
        )
        # return td, tmp
...
class TreeModule(torch.nn.Module, Transformer):
    ...
    def number(self, value):
        return float(
            value.value
        )  # torch.scalar_tensor(float(value.value), dtype=torch.float32)
    ...
    def extract(self, name):
        ...
        self.seq.add_module(
            f"extract{self.extracted}", NamedTensor(key, self.extracted)
        )
        self.extracted += 1
        return key
    ...
    def names(self, *args):
        ...
        already_extracted = set(
            map(lambda m: m.key, filter(lambda m: isinstance(m, NamedTensor), self.seq))
        )
    ...
    def slicing(self, token_or_tree, *args):
        ...
        for i, (a, d) in enumerate(filter(lambda ad: ad[0] != ":", zip(args, dims))):
            ...
            if isinstance(a, Token):
                ...
                self._slice(
                    k, d, list(map(int, a.children))
                )  # NOTE: error if >2 children
            else:
                raise RuntimeError(
