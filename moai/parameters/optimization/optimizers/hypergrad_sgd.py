...
class HypergradSGD(torch.optim.Optimizer):
    ...
    def __init__(
        ...
        defaults = dict(
            ...
            hypergrad_momentum=hypergrad_momentum,
            min_lr=min_lr,
        )
        ...
        for group in self.param_groups:
            group["prev_grads"] = [
                torch.zeros_like(p, requires_grad=False) for p in group["params"]
            ]
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for index, (p, d_p_prev) in enumerate(
                zip(group["params"], group["prev_grads"])
            ):
                if p.grad is None:
                    continue
                ...
                if hypergrad_momentum != 0:
                    ...
                    if "momentum_buffer_h" not in param_state:
                        buf_h = param_state["momentum_buffer_h"] = torch.clone(
                            h
                        ).detach()
                    else:
