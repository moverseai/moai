...
def assert_numeric(
    ...
    if (
        ...
        logger.error(
            f"Parameter {name} (value: {value}) should be in range [{min_value}, {max_value}]."
        )
    if min_value is not None and max_value is None and value < min_value:
        logger.error(
            f"Parameter {name} (value: {value}) should be larger than {min_value}."
        )
    if max_value is not None and min_value is None and value > max_value:
        logger.error(
            f"Parameter {name} (value: {value}) should be smaller than {max_value}."
        )
