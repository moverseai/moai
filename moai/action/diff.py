...
def report_hydra(ddiff: dd.DeepDiff) -> None:
    ...
    def format_added_item(txt: str) -> str:
        ret = (
            txt.replace("'", "").replace("root", "").replace("[", "").replace("]", ".")
        )
...
def report_rich(ddiff: dd.DeepDiff, console: Console) -> None:
    ...
    if results["item"]["added"]:
        ...
        for i, (a, r) in enumerate(
            itertools.zip_longest(results["item"]["added"], results["item"]["removed"])
        ):
            key_table.add_row(
    ...
    if collection_diffs:
        ...
        for k in collection_diffs:
            ...
            collection_table.add_row(
                f"[bold blue] {k}", f"[bold red] {r}", f"[bold green] {a}"
            )
    if collection_added:
        for k in collection_added:
            a = results["collection"]["added"][k]
            collection_table.add_row(f"[bold blue] {k}", "", f"[bold green] {a}")
...
def diff(cfg):
    ...
    console.line()
    console.print(
        "[bold red]From:[/bold red] :arrow_right_hook:", cfg.lhs, style=from_style
    )
    # console.rule()
    to_style = rich.style.Style(frame=True)
    console.print(
        "[bold green]To:[/bold green] ", cfg.rhs, justify="right", style=to_style
    )
    console.line()
    # console.rule()
