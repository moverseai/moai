    ...
    if val_csv:
        ...
        if options.epoch_mode == "best":
            ...
            for k, v in metric_names.items():
                ...
                best_metrics[name] = (
                    metrics[name].max(),
                    metrics["epoch"][metrics[name].idxmax()],
                )
            ...
            epoch_value = (
                [0, sys.maxsize]
                if options.epoch_mode == "all"
                else ensure_numeric_list(options.epoch_value)
            )
            min_epoch, max_epoch = (
                tuple(epoch_value)
                if isinstance(epoch_value, omegaconf.ListConfig)
                and len(epoch_value) == 2
                else (epoch_value[0], epoch_value[0])
            )
        ...
        log.error(
            f"Could not find a validation metrics .csv file in {path}, skipping metrics for this experiment."
        )
    return result
...
def extract_optimization(
    ...
    if optimizers and isinstance(optimizers, dict):
        if len(optimizers) == 1 and "lr" in toolz.first(optimizers.values()):
            learning_rates[toolz.first(optimizers.keys())] = toolz.first(
                optimizers.values()
            )["lr"]
        else:
...
def extract_data(
    path: str, plot_config: omegaconf.DictConfig
    ...
    if plot_config.metrics:
        data_frame["metrics"] = extract_metrics(
            moai_config, path, plot_config.metrics_options
        )
...
def post_process(
    data: typing.List[
        typing.Dict[str, typing.Dict[str, typing.Union[str, float, int]]]
    ],
    config: omegaconf.DictConfig,
) -> typing.List[typing.Dict[str, typing.Union[str, float, int]]]:
    if config.losses:
        log.info("Adding losses ...")
        loss_keys = functools.reduce(
            lambda s, d: s | set(safe_get_dict("losses", d).keys()), data, set()
        )
        losses_empty = dict(((k, 0.0) for k in loss_keys))
    if config.metrics:
        log.info("Adding metrics ...")
        metric_keys = functools.reduce(
            lambda s, d: s | set(safe_get_dict("metrics", d).keys()), data, set()
        )
        metrics_empty = dict(((k, 0.0) for k in metric_keys))
    if config.monads:
        log.info("Adding monads ...")
        monads_keys = functools.reduce(
            lambda s, d: s | set(safe_get_dict("monads", d).keys()), data, set()
        )
...
def plot(cfg):
    from_date = parse_human_date_time(
        cfg.filter.date.start.day, cfg.filter.date.start.time
    )
    to_date = parse_human_date_time(cfg.filter.date.end.day, cfg.filter.date.end.time)
    if from_date > to_date:
        log.error(
            "Invalid date ranges, start date ({from_date}) should be earlier than end date ({to_date}) date."
        )
        return
    ...
    root = (
        ...
        else os.path.join(hydra.utils.get_original_cwd(), root)
    )
    for date_folder in filter(
        lambda d: os.path.isdir(os.path.join(root, d)), os.listdir(root)
    ):
        for day_folder in filter(
            ...
            date_paths.append(
                (combined, date, os.path.join(root, date_folder, day_folder))
            )
    paths = [
        (datetime, folder)
        for (datetime, date, folder) in date_paths
...
if __name__ == "__main__":
    ...
    sys.argv.append(
        "hydra.run.dir=actions/${hydra.job.name}/${now:%Y-%m-%d}/${now:%H-%M-%S}"
    )
    plot()
