...
try:
    ...
    log.warning(
        "The `blobconverter` package was not found, exporting to OpenVivo is disabled."
    )
...
class TraceWrapper(pl.LightningModule):
    ...
    def forward(
        ...
        for module in self.exported_module_names:
            try:
                m = get_submodule(self, module) if module != "model" else self.model
            except:
                log.error(f"The {m} module does not exist in your model!")
                log.info(
                    f"Please select on of the following {*self.model._modules.keys(),}"
                )
            x = m(x)
...
def export(cfg):
    ...
    device = cfg.tester.gpus[0] if "tester" in cfg.keys() else cfg.trainer.gpus[0]
    model.eval()
    model.initialize_parameters()
    model.to(device)
    try:
        checkpoint = pl_load(
            cfg.model.parameters.initialization.filename,
            map_location=lambda storage, loc: storage,
        )
    except:
        log.error(
            " You are exporting a model without passing pretrained weights "
            "\n Include them, by adding '-parameters.initialization.filename: <your_path>'"
        )
    trwrapper = TraceWrapper(
        cfg.export.module_names, cfg.export.output_names, cfg.export.input_names, model
    )
    trwrapper.eval()
    trwrapper.to(device)
    input_dict = {}
    ...
    if cfg.export.mode == "onnx":
        try:
            ...
            moai_url = get_command_output(
                ["git", "ls-remote", "--get-url", "origin"], path, strip=True
            )
            moai_commit = get_command_output(
                ["git", "rev-parse", "HEAD"], path, strip=True
            )
        ...
        try:
            ...
            log.warning(
                "You have not included other paths and thus local repository info will not be included in the metadata."
            )
        if other_paths:
        ...
        if moai_url:  # add moai repo details
            model.metadata_props.append(
                onnx.StringStringEntryProto(key="moai", value=str(moai_url))
            )
            model.metadata_props.append(
                onnx.StringStringEntryProto(key="moai-commit", value=str(moai_commit))
            )
        if project_path_urls:  # add repo details
            for url, commit in zip(project_path_urls, project_path_commits):
                model.metadata_props.append(
                    onnx.StringStringEntryProto(key="repo", value=str(url))
                )
                model.metadata_props.append(
                    onnx.StringStringEntryProto(key="repo-commit", value=str(commit))
                )
        try:
            for key in cfg.onnx.metadata:
                v = toolz.get_in(key.split("."), checkpoint)
                model.metadata_props.append(
                    onnx.StringStringEntryProto(key=key, value=str(v))
                )
        except:
        ...
        try:
            for key in cfg.onnx.attributes:
                try:
                    ...
                    log.warning(
                        f" Attribute {key} does not supported in existing onnx version."
                    )
        except:
...
if __name__ == "__main__":
    ...
    sys.argv.append(
        "hydra.run.dir=actions/${hydra.job.name}/${now:%Y-%m-%d}/${now:%H-%M-%S}-${export.name}"
    )
    export = hydra.main(
