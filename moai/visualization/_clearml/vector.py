...
class Vector(object):
    ...
    def __call__(
        ...
        for v, t in zip(self.vector, self.types):
            vector = v(tensors)
            b = int(math.ceil(self.batch_percentage * vector.shape[0]))
            for i in range(b):
                self.viz_map[t](
                    vector[i].detach().cpu().numpy(), f"boxplot_{i}", i, t, self.xaxis
                )
    ...
    def _viz_box(
        ...
        logger.report_vector(
            title=env, series=f"{key}_{step}", values=array, iteration=step, xaxis=xaxis
        )
