...
class Pose2d(object):
    def __init__(
        ...
        self.viz_pose = {
            "human_pose2d": functools.partial(
                self.__draw_human_pose2d,
                self.logger,
            ...
            * len(self.pred),
            ...
            self.viz_pose[poses](
                ...
                (
                    gt_masks.detach()
                    if self.use_mask
                    else torch.ones_like(gt_coord[..., 0:1])
                ),
                (
                    pred_masks.detach()
                    if self.use_mask
                    else torch.ones_like(pred_coord[..., 0:1])
                ),
                pose_struct,
    ...
    def __draw_human_pose2d(
        ...
        imgs = (
            np.zeros([b, 3, int(scale * h), int(scale * w)], dtype=np.uint8)
            if not rotate
