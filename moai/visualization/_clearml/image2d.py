...
class Image2d(object):
            ...
            self.viz_map[t](
                self.colorize_map[c](
                    self.transform_map[tf](
                        tensors,
                        k,
                        int(math.ceil(self.batch_percentage * tensors[k].shape[0])),
                    )
                ),
                k,
                step,
    ...
    @staticmethod
    def _viz_heatmap(
        logger: clearml.Logger, tensor: torch.Tensor, key: str, win: str, env: str
    ) -> None:
        b, _, __, ___ = tensor.shape  # NOTE: assumes [B, C, H, W], i.e. 2d train
        heatmaps = np.flipud(tensor)
        for i in range(b):
            opts = {"title": key + "_" + str(i), "colormap": "Viridis"}
    ...
    def _dataset_normalization(
        ...
        return torch.addcmul(mean, tensor, std)
