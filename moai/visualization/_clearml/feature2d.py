...
class Feature2d(object):
    def __init__(
        ...
        self.colormaps = (
            [colormap] if isinstance(colormap, str) is str else list(colormap)
        )
        self.viz_map = {
            "color": functools.partial(
                self._viz_color, self.logger, max_history=max_history
            ),
    ...
    def __call__(
        ...
        for k, t, tf, c in zip(
            self.images, self.types, self.transforms, self.colormaps
        ):
            if not k:
                continue
            tensor = tensors[k]
            ...
            for i in range(h):
                ...
                self.viz_map[t](
                    self.colorize_map[c](
                        self.transform_map[tf](tensor[:, i, ...].unsqueeze(1))
                    ),
                    key,
                    step,
                    k,  # self.name
    ...
    @staticmethod
    def __viz_heatmap(
        visdom: visdom.Visdom, tensor: torch.Tensor, key: str, win: str, env: str
    ) -> None:
        b, _, __, ___ = tensor.size()  # assumes [B, C, H, W], i.e. 2d train
        heatmaps = torch.flip(tensor, dims=[2]).detach().cpu()
        for i in range(b):
            opts = {"title": key + "_{}".format(i), "colormap": "Viridis"}
