...
class RenderedMesh(Image2d):
    def __init__(
        ...
        super(RenderedMesh, self).__init__(
            ...
            type=list(
                itertools.repeat(
                    "color", len([vertices] if isinstance(vertices, str) else vertices)
                )
            ),
            transform=transform,
        ...
        self.focal_length = (
            ...
            if isinstance(focal_length, float) or isinstance(focal_length, int)
            else focal_length
        )
        ...
        self.translation = list(
            itertools.repeat("", len(self.keys))
            if translation is None
            else ([translation] if isinstance(translation, str) else list(translation))
        )
        self.rotation = list(
            itertools.repeat("", len(self.keys))
            if rotation is None
            else ([rotation] if isinstance(rotation, str) else list(rotation))
        )
        self.scale = scale
        self.renderer = None
    ...
    def _get_renderer(self, width: int, height: int) -> pyrender.OffscreenRenderer:
        if (
            ...
            or self.renderer.viewport_width != width
    ...
    def __call__(
        ...
        for v, f, r, t, k, _, tf, c, fmt in zip(
            ...
            background = self.colorize_map[c](self.transform_map[tf](tensors, k, take))
            b, c, h, w = background.shape
            renderer = self._get_renderer(width=w, height=h)
            results = []
            for i in range(b):
                rotation = (
                    tensors[r][i].detach().cpu().numpy().squeeze() if r else np.eye(3)
                )
                translation = (
                    tensors[t][i].detach().cpu().numpy().squeeze() if t else np.zeros(3)
                )
                ...
                input_img = (
                    background.detach().cpu().numpy().squeeze().transpose(1, 2, 0)
                )
                output_img = (
                    color[:, :, :-1] * valid_mask + (1 - valid_mask) * input_img
                )
                if self.scale != 1.0:
                    output_img = np.array(
                        Image.fromarray((output_img * 255.0).astype(np.uint8)).resize(
            ...
            self.save_map["color"](
                np.stack(results).transpose(0, 3, 1, 2), f"{k}_overlay", self.index, fmt
            )
