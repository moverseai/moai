...
class Mesh(
    ...
    def __init__(
        ...
        self.formats = [
            ensure_choices(log, "output format", ext, Mesh.__FORMATS__)
            for ext in filetype
        ]
    ...
    def __call__(
        ...
        for n, v, f, ext in zip(self.names, self.vertices, self.faces, self.formats):
            verts = v(tensors).detach().cpu().numpy()
            indices = f(tensors).detach().cpu().numpy()
            bs = verts.shape[0]
            for i in range(bs):
                trimesh.Trimesh(verts[i], indices[i], process=False).export(
                    os.path.join(
...
class PointCloud(
    ...
    def __init__(
        ...
        self.folder = ensure_path(log, "output folder", path)
        self.formats = [
            ensure_choices(log, "output format", ext, PointCloud.__FORMATS__)
            for ext in filetype
        ]
        self.names = [vertices] if isinstance(vertices, str) else list(vertices)
        self.vertices = [_create_accessor(k) for k in self.names]
    ...
    def __call__(self, tensors: typing.Dict[str, torch.Tensor]) -> None:
        for n, v, ext in zip(self.names, self.vertices, self.formats):
            ...
            bs = verts.shape[0]
            for i in range(bs):
                trimesh.points.PointCloud(verts[i], colors=None, metadata=None).export(
                    os.path.join(
                        self.folder, f"{n}_{(self.index + i):{self.format}}.{ext}"
                    )
                )
