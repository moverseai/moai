...
class RenderFunction(torch.autograd.Function):
    ...
    def serialize_scene(scene: pyredner.Scene,
        ...
        if not cam.position is None or torch.isfinite(cam.position).all():
            log.warning("Camera position is not finite !")
        if not cam.look_at is None or torch.isfinite(cam.look_at).all():
            log.warning("Camera look at is not finite !")
        if not cam.up is None or torch.isfinite(cam.up).all():
