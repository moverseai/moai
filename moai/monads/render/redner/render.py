...
def serialize_texture(texture, args, device):
    ...
    for mipmap in texture.mipmap:
        ...
        if mipmap.device != device:
            warnings.warn(
                "Converting texture from {} to {}, this can be inefficient.".format(
                    mipmap.device, device
                )
            )
...
class RenderFunction(torch.autograd.Function):
    ...
    def serialize_scene(
        ...
        for shape in scene.shapes:
            ...
            if shape.vertices.device != device:
                warnings.warn(
                    "Converting shape vertices from {} to {}, this can be inefficient.".format(
                        shape.vertices.device, device
                    )
                )
            if shape.indices.device != device:
                warnings.warn(
                    "Converting shape indices from {} to {}, this can be inefficient.".format(
                        shape.indices.device, device
                    )
                )
            ...
            args.append(
                shape.uv_indices.to(device) if shape.uv_indices is not None else None
            )
            args.append(
                shape.normal_indices.to(device)
                if shape.normal_indices is not None
                else None
            )
    ...
    @staticmethod
    def unpack_args(
        seed, args, use_primary_edge_sampling=None, use_secondary_edge_sampling=None
    ):
        """
        ...
        if cam_to_world is None:
            camera = redner.Camera(
                ...
                redner.float_ptr(
                    distortion_params.data_ptr() if distortion_params is not None else 0
                ),
                clip_near,
                camera_type,
        ...
        for i in range(num_shapes):
            ...
            shapes.append(
                redner.Shape(
                    ...
                    redner.int_ptr(
                        uv_indices.data_ptr() if uv_indices is not None else 0
                    ),
                    redner.int_ptr(
                        normal_indices.data_ptr() if normal_indices is not None else 0
                    ),
        ...
        for i in range(num_materials):
            ...
            if len(generic_texture) > 0:
                ...
                generic_texture = redner.TextureN([], [], [], 0, redner.float_ptr(0))
            ...
            if len(normal_map) > 0:
                ...
                normal_map = redner.Texture3([], [], [], 0, redner.float_ptr(0))
            materials.append(
                redner.Material(
                    diffuse_reflectance,
                    specular_reflectance,
    ...
    def create_gradient_buffers(ctx):
        ...
        if camera.use_look_at:
            buffers.d_camera = redner.DCamera(
                ...
                redner.float_ptr(
                    buffers.d_distortion_params.data_ptr()
                    if buffers.d_distortion_params is not None
                    else 0
                ),
            )
        else:
        ...
        for shape in ctx.shapes:
            ...
            d_uvs = (
                torch.zeros(num_uv_vertices, 2, device=device)
                if shape.has_uvs()
                else None
            )
            d_normals = (
                torch.zeros(num_normal_vertices, 3, device=device)
                if shape.has_normals()
                else None
            )
            d_colors = (
                torch.zeros(num_vertices, 3, device=device)
                if shape.has_colors()
                else None
            )
            ...
            buffers.d_shapes.append(
                redner.DShape(
                    redner.float_ptr(d_vertices.data_ptr()),
                    redner.float_ptr(d_uvs.data_ptr() if d_uvs is not None else 0),
                    redner.float_ptr(
                        d_normals.data_ptr() if d_normals is not None else 0
                    ),
                    redner.float_ptr(
                        d_colors.data_ptr() if d_colors is not None else 0
                    ),
                )
            )
        ...
        for material in ctx.materials:
            if material.get_diffuse_size(0)[0] == 0:
                ...
                for l in range(material.get_diffuse_levels()):
                    ...
                    d_diffuse.append(
                        torch.zeros(diffuse_size[1], diffuse_size[0], 3, device=device)
                    )
            ...
            if material.get_specular_size(0)[0] == 0:
                ...
                for l in range(material.get_specular_levels()):
                    ...
                    d_specular.append(
                        torch.zeros(
                            specular_size[1], specular_size[0], 3, device=device
                        )
                    )
            ...
            if material.get_roughness_size(0)[0] == 0:
                ...
                for l in range(material.get_roughness_levels()):
                    ...
                    d_roughness.append(
                        torch.zeros(
                            roughness_size[1], roughness_size[0], 1, device=device
                        )
                    )
            ...
            if material.get_normal_map_levels() == 0:
                ...
                for l in range(material.get_normal_map_levels()):
                    ...
                    d_normal_map.append(
                        torch.zeros(
                            normal_map_size[1], normal_map_size[0], 3, device=device
                        )
                    )
            ...
            if d_generic is None:
                d_generic_tex = redner.TextureN([], [], [], 0, redner.float_ptr(0))
            else:
            ...
            if d_normal_map is None:
                d_normal_map = redner.Texture3([], [], [], 0, redner.float_ptr(0))
            else:
        ...
        if ctx.envmap is not None:
            ...
            for l in range(envmap.get_levels()):
                ...
                buffers.d_envmap_values.append(
                    torch.zeros(size[1], size[0], 3, device=device)
                )
            ...
            buffers.d_envmap = redner.DEnvironmentMap(
                d_envmap_tex, redner.float_ptr(buffers.d_world_to_env.data_ptr())
            )
    ...
    def visualize_screen_gradient(
        ...
        img_height = viewport[2] - viewport[0]
        img_width = viewport[3] - viewport[1]
        screen_gradient_image = torch.zeros(img_height, img_width, 2, device=device)
...
class AmbientLight(DeferredLight):
    ...
    def render(
        self, position: torch.Tensor, normal: torch.Tensor, albedo: torch.Tensor
    ):
...
class PointLight(DeferredLight):
    ...
    def __init__(self, position: torch.Tensor, intensity: torch.Tensor):
    ...
    def render(
        ...
        return (
            self.intensity.to(dot_l_n.device)
            * dot_l_n
            * (albedo / math.pi)
            / light_dist_sq
        )
...
class DirectionalLight(DeferredLight):
    ...
    def __init__(self, direction: torch.Tensor, intensity: torch.Tensor):
...
class SpotLight(DeferredLight):
    ...
    def render(
        ...
        dot_l_n = torch.max(dot_l_n, torch.zeros_like(dot_l_n))
        return (
            self.intensity.to(spot_factor.device)
            * spot_factor
            * dot_l_n
            * (albedo / math.pi)
        )
...
def render_deferred(
    ...
    if isinstance(scene, pyredner.Scene):
        ...
        scene.camera.resolution = (org_res[0] * aa_samples, org_res[1] * aa_samples)
        ...
        if aa_samples > 1:
            ...
            if org_viewport is not None:
                org_size = (
                    org_viewport[2] - org_viewport[0],
                    org_viewport[3] - org_viewport[1],
                )
            else:
                org_size = org_res
        ...
        if len(lights) > 0 and not isinstance(lights[0], list):
            ...
            for sc, se in zip(scene, seed):
                ...
                sc.camera.resolution = (
                    org_res[0] * aa_samples,
            ...
            for sc, se, lgts in zip(scene, seed, lights):
                ...
                albedo = g_buffer[:, :, 6:9]
                img = torch.zeros(
                    g_buffer.shape[0], g_buffer.shape[1], 3, device=device
                )
                for light in lgts:
                    img = img + light.render(pos, normal, albedo)
                if alpha:
