...
try:
    ...
    log.error(
        f"The pyredner package (https://github.com/BachiLi/redner) is required to use the corresponding rendering monads. Install with `pip install pyredner`"
    )

RenderParams = namedtuple(
    "RenderParams",
    [
        "samples",
        "sample_pixel_center",
...
class Silhouette(torch.nn.Module):
    def __init__(
        ...
        self.params = RenderParams(
            ...
            principal_point=(
                (float(principal_point), float(principal_point))
                if isinstance(principal_point, (float, int))
    ...
    def forward(
        ...
        if intrinsics is None:
            with torch.no_grad():
                ...
                intrinsics = (
                    torch.eye(3, device=vertices.device)[np.newaxis, ...]
                    .expand(b, 3, 3)
                    .clone()
                )
                intrinsics[:, 0, 0] = (
                    self.params.focal_length[0] / ndc_width
                ) * width_scale
                intrinsics[:, 1, 1] = (
                    self.params.focal_length[1] / ndc_width
                ) * height_scale
                intrinsics[:, 0, 0] *= 4.0
                intrinsics[:, 1, 1] *= 4.0
                # intrinsics[:, 0, 2] = (
                ...
                intrinsics[:, 0, 2] = (
                    (
                        (
                            0.5 * resolution[-1]
                            - self.params.principal_point[0] * width_scale
                        )
                        / (0.5 * resolution[-1])
                        / 1.0
                    )
                ...
                intrinsics[:, 1, 2] = (
                    (
                        (
                            0.5 * resolution[-2]
                            - self.params.principal_point[1] * height_scale
                        )
                        / (0.5 * resolution[-1])
                        / 1.0
                    )
