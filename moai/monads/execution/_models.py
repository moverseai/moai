...
class Criteria:
    __PRESET_ARGS__ = set(
        ["step", "epoch", "batch_idx", "optimization_step", "stage", "iter"]
    )
    ...
    class ArgsOperation:
        ...
        def __call__(
            ...
            for i, args in enumerate(self.tensor_args):
                kwargs = toolz.valmap(
                    lambda a: a(tensors).detach().cpu().numpy().squeeze(), args
                )
    ...
    def _get_tensor_args(
        self, args: typing.Mapping[str, inspect.Parameter]
    ) -> typing.Mapping[
        ...
        **kwargs: typing.Mapping[str, typing.Any],
        ...
        for k in kwargs or {}:
            ...
            if self.is_first_arg_tensor_dict(
                ...
                args = (
                    set(toolz.drop(1, signature.parameters.keys()))
                    - Tensors.__PRESET_ARGS__
                )
                op_args = toolz.keyfilter(
                    lambda k: k in args, params
                )  # NOTE: currently assumes that no arg is named `params`
                self.operations.append(
                    Tensors.DictOperation(operation, op_args, extras)
                )
            else:
        ...
        **kwargs: typing.Mapping[str, typing.Any],
        ...
        for k, p in loop:
            ...
            for keys in zip(*list(p[prop] for prop in itertools.chain(props, ["out"]))):
                accessors = [
                    _create_accessor(k if isinstance(k, str) or k is None else k[0])
                    for k in keys[:-1]
                ]
                self.execs.append(
                    lambda tensor_dict, acc=accessors, k=keys, p=props, f=module: tensor_dict.update(
                        {
                            k[-1]: f(
                                **dict(
                                    zip(
