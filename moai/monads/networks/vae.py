...
class VAE(torch.nn.Module):
    def __init__(
        ...
        model = toolz.dissoc(
            hparams["model"], "supervision", "generation", "validation", "feedforward"
        )
        self.model = hyu.instantiate(model)
        self.model.encoder.load_state_dict(
            toolz.keymap(
                lambda s: s.replace("encoder.", ""),
                toolz.keyfilter(lambda s: s.startswith("encoder."), ckpt["state_dict"]),
            )
        )

        self.model.latent_dist_predictor.load_state_dict(
            toolz.keymap(
                lambda s: s.replace("latent_dist_predictor.", ""),
                toolz.keyfilter(
                    lambda s: s.startswith("latent_dist_predictor."), ckpt["state_dict"]
                ),
            )
        )
        ...
        self.model.sampler.load_state_dict(
            toolz.keymap(
                lambda s: s.replace("sampler.", ""),
                toolz.keyfilter(lambda s: s.startswith("sampler."), ckpt["state_dict"]),
            )
        )

        self.model.decoder.load_state_dict(
            toolz.keymap(
                lambda s: s.replace("decoder.", ""),
                toolz.keyfilter(lambda s: s.startswith("decoder."), ckpt["state_dict"]),
            )
        )
