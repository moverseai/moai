...
class GAN(torch.nn.Module):
    def __init__(self,
        ...
        self.model.generator.load_state_dict(
            toolz.keymap(lambda s: s.replace('generator.', ''), 
                toolz.keyfilter(
                    lambda s: s.startswith('generator.'), 
                    ckpt['state_dict']
                )
            )
        )
        ...
        self.model.discriminator.load_state_dict(
            toolz.keymap(lambda s: s.replace('discriminator.', ''), 
                toolz.keyfilter(
                    lambda s: s.startswith('discriminator.'), 
                    ckpt['state_dict']
                )
            )
        )
