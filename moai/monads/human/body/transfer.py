...
class BodyTransfer(torch.nn.Module):
    def __init__(
        ...
        if ext == ".pkl":
            ...
            log.error(
                f"Unsupported extension ({ext}) to load body tranfer weights from when loading {weights_path}. Can only load *.pkl, *.npy, *.npz files"
            )
        self.register_buffer(
            "weights", torch.from_numpy(w).float(), persistent=persistence
        )
