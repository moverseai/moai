...
__JOINT__MAPPERS__ = {
    "none": None,
    "openpose_coco25": functools.partial(
        JointMap,
        model="smplx",
        format="coco25",
        with_hands=True,
        with_face=True,
        with_face_contour=False,
    ),
    "openpose_coco25_face": functools.partial(
        JointMap,
        model="smplx",
        format="coco25",
        with_hands=True,
        with_face=True,
        with_face_contour=True,
    ),
    "openpose_coco19": functools.partial(
        JointMap,
        model="smplx",
        format="coco19",
        with_hands=True,
...
class SMPLX(smplx.SMPLX):
    def __init__(
        ...
        super(SMPLX, self).__init__(
            ...
            joint_mapper=(
                None
                if joints_format is None
                else mapper() if mapper is not None else None
            ),
    ...
    def forward(
        ...
        beta_coeffs = (
            ...
            if shape.shape[1] == self.num_betas
                ...
                [
                    shape,
                    torch.zeros(shape.shape[0], self.num_betas - shape.shape[1]).to(
                        shape
                    ),
                ],
                dim=1,
