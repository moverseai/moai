...
class JointRegressor(torch.nn.Module):
    def __init__(
        ...
        if weights_path:
            ...
            if ext == ".npz":
                ...
                log.error(
                    f"Unsupported extension ({ext}) for loading the joint regressor weights @ {weights_path}."
                )
        else:
            log.info("Using a joint regressor with no persistent weights.")
            weights = np.array([])
        self.register_buffer(
            "weights", torch.from_numpy(weights).float(), persistent=persistence
        )
    ...
    def forward(
        ...
        return torch.einsum(
            "bvc,jv->bjc", vertices, self.weights if regressor is None else regressor
        )
