...
class Interpolate(torch.nn.Module):
    ...
    STATIC_PARAMS = namedtuple(
        "params", ["mode", "align_corners", "recompute_scale_factor"]
    )
    ...
    def __init__(
        ...
        if not scale > 0.0 and not (width > 0 and height > 0):
            log.error(
                f"Either scale ({scale}) or dimensions ([{width} x {height}]) need to be set for Interpolate."
            )
        self.func = (
            functools.partial(
                ...
                mode=mode,
                align_corners=(
                    align_corners if mode != "nearest" and mode != "area" else None
                ),
                recompute_scale_factor=recompute_scale_factor,
            )
            if scale > 0.0 and width < 0 and height < 0
            else functools.partial(
                torch.nn.functional.interpolate,
                size=self._get_tuple((height, width)),
                mode=mode,
                align_corners=(
                    align_corners if mode != "nearest" and mode != "area" else None
                ),
        ...
        self.params = Interpolate.STATIC_PARAMS(
            mode, align_corners, recompute_scale_factor
        )
    ...
    def forward(
        ...
        return (
            self.func(image)
            if target is None and not self.preserve_aspect_ratio
...
BilinearDownsample_x2 = functools.partial(
    Interpolate,
    mode="bilinear",
    scale=0.5,
)
NearestDownsample_x2 = functools.partial(
    Interpolate,
    mode="nearest",
    scale=0.5,
)
BilinearSampling = functools.partial(
    Interpolate,
    mode="bilinear",
)
NearestSampling = functools.partial(
    Interpolate,
    mode="nearest",
)
NearestUpsample_x2 = functools.partial(Interpolate, scale=2.0, mode="nearest")
BilinearUpsample_x2 = functools.partial(Interpolate, scale=2.0, mode="bilinear")
...
class ScalePyramid(torch.nn.Module):
    def __init__(
        ...
        if scales is None and sizes is None:
            log.error(
                "When using a scale_pyramid one of `scale` or `sizes` configuration parameters is required."
            )
        if scales is not None and sizes is not None:
            log.error(
                "When using a scale_pyramid ONLY ONE of `scale` or `sizes` configuration parameters is required."
            )
