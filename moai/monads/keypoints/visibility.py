...
__all__ = ["VisibilityFOV", "VisibilityHeatmap"]
...
class VisibilityFOV(torch.nn.Module):
    ...
    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        if self.coord_type != "coord":
            _coords[..., 0] = (
                (_coords[..., 0] + 1.0) / 2.0 * self.width
                if self.coord_type == "ndc"
                else _coords[..., 0] * self.width
            )
            _coords[..., 1] = (
                (_coords[..., 1] + 1.0) / 2.0 * self.height
                if self.coord_type == "ndc"
                else _coords[..., 1] * self.height
            )
        ...
        for i in range(channels):
            ...
            for b in range(coords.shape[0]):
                ...
                if (
                    uv[0] > -1
                    and uv[1] > -1
                    and uv[0] < heatmap.shape[-2]
                    and uv[1] < heatmap.shape[-1]
                ):
