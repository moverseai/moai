...
class NormToCoords(torch.nn.Module):
    ...
    def forward(self, coords: torch.Tensor, img: torch.Tensor) -> torch.Tensor:
        if self.flip:
            coords = coords * torch.Tensor([*img.shape[2:]]).flip(-1).to(
                coords
            ).expand_as(coords)
        else:
            coords = coords * torch.Tensor([*img.shape[2:]]).to(coords).expand_as(
                coords
            )
        return coords
...
class NormToNdc(torch.nn.Module):
    ...
    def forward(self, coords: torch.Tensor, img: torch.Tensor) -> torch.Tensor:
        if self.flip:
            ndc = (
                torch.addcmul(
                    torch.scalar_tensor(-1.0).to(coords),
                    coords,
                    torch.scalar_tensor(2.0).to(coords),
                ).flip(-1),
            )  # from [0, 1] to [-1, 1]
        else:
            ndc = (
...
class NdcToCameraCoords(torch.nn.Module):
    ...
    def forward(self, coords: torch.Tensor) -> torch.Tensor:
        if self.order == "xy":
            camera_coords = torch.div(
                coords,
                torch.tensor([self.aspect_ratio / self.tanfov, (1 / self.tanfov)]).to(
                    coords
                ),
            )
        else:
            camera_coords = torch.div(
                coords,
                torch.tensor([(1 / self.tanfov), self.aspect_ratio / self.tanfov]).to(
                    coords
                ),
            )
...
class CoordsToNorm(torch.nn.Module):
    ...
    def forward(
        ...
        dims = torch.tensor(
            dims_from.shape[2:], dtype=torch.float32, device=coords.device
        )
        if self.flip:
            dims = dims.flip(-1)
...
UpscaleCoords_x2 = functools.partial(ScaleCoords, scale=2)

UpscaleCoords_x4 = functools.partial(ScaleCoords, scale=4)

DownscaleCoords_x2 = functools.partial(ScaleCoords, scale=0.5)

DownscaleCoords_x4 = functools.partial(ScaleCoords, scale=0.25)
