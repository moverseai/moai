...
class Projection(torch.nn.Module):
    ...
    def __init__(
        ...
        self.homogemize = (
            self._homogemize_channel if xyz_at == "channel" else self._homogenize_row
        )
        self.format_uv = (
            self._format_uv_channel if uv_at == "channel" else self._format_uv_row
        )
        self.convert_uv = (
            self._convert_ndc
            if uv_type == "ndc"
    ...
    def _convert_norm(
        ...
        dims = torch.stack(
            [torch.Tensor([[width]]), torch.Tensor([[height]])], dim=1
        ).to(uvs)
        return uvs[:, :2, ...] / dims
