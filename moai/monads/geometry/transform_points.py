...
class Transformation(torch.nn.Module):
    ...
    def __init__(
        ...
        self.convert_points_in = (
            self._convert_points_row_in
            if xyz_in_at == "row"
            else self._convert_points_channel_in
        )
        self.convert_points_out = (
            self._convert_points_row_out
            if xyz_out_at == "row"
            else self._convert_points_channel_out
        )
        self.get_width_height = (
            self._get_width_height_row_in
            if xyz_in_at == "row"
            else self._get_width_height_channel_in
        )

    def _convert_points_row_in(
        self,
        points: torch.Tensor,
    ):
        return (
    ...
    def _convert_points_channel_in(
        self,
        points: torch.Tensor,
    ):
        return points

    def _get_width_height_row_in(
        self,
        shape: torch.Size,
    ):
        return shape[2], shape[1]

    def _get_width_height_channel_in(
        self,
        shape: torch.Size,
    ):
        return shape[3], shape[2]

    def _convert_points_channel_out(
        self,
        points: torch.Tensor,
        shape: torch.Size,
    ):
        return (
            points
            if len(shape) == 3
            else points.reshape(points.shape[0], *self.get_width_height(shape), 3)
        )

    def _convert_points_row_out(
        self,
        points: torch.Tensor,
        shape: torch.Size,
    ):
        ...
        return (
            pts
            if len(shape) == 3
            else points.reshape(points.shape[0], 3, *self.get_width_height(shape))
        )
