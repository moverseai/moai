...
__MODE_CONVERTERS__ = {
    "ndc": lambda g, _: torch.addcmul(
        torch.scalar_tensor(-1.0), g, torch.scalar_tensor(2.0)
    ),  # from [0, 1] to [-1, 1]
...
class Grid(torch.nn.Module):
    ...
    def __init__(
        ...
        unit_grid = _create_inclusive(size) if inclusive else _create_exclusive(size)
        grid = __MODE_CONVERTERS__[mode](unit_grid, size)
        order = order[: len(size)]
        size_diff = len(Grid.__XYZ__) - len(order)
        indices = [
            Grid.__XYZ__.index(c) - size_diff for c in order if c in Grid.__XYZ__
        ]
        grid = torch.stack([grid[:, i, ...] for i in indices], dim=1)
...
class MultiScaleGrid(torch.nn.Module):
    def __init__(
        ...
        for i, size in enumerate(sizes):
            unit_grid = (
                _create_inclusive(size) if inclusive else _create_exclusive(size)
            )
            grid = __MODE_CONVERTERS__[mode](unit_grid, size)
            grid = torch.flip(grid, dims=[1])
            self.register_buffer(
                "x".join(map(str, reversed(size))), grid, persistent=persistent
            )
