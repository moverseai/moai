...
class PerBatchCallback(pytorch_lightning.Callback):
    ...
    def on_train_batch_end(
        ...
        if trainer.global_step and (
            trainer.global_step % pl_module.visualization.interval == 0
        ):
            pl_module.visualization(outputs["tensors"], trainer.global_step)
        if trainer.global_step and (
            trainer.global_step % pl_module.exporter.interval == 0
        ):
    ...
    def on_test_batch_end(
        ...
        metrics, tensors = outputs
        if pl_module.global_test_step and (
            trainer.global_step % pl_module.exporter.interval == 0
        ):
            pl_module.exporter(tensors)
        if pl_module.global_test_step and (
            trainer.global_step % pl_module.visualization.interval == 0
        ):
            pl_module.visualization(tensors)
        return tensors
