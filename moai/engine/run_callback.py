...
class RunCallback(L.Callback):
    ...
    def on_train_epoch_start(
        ...
        if module.schedule and module.current_epoch >= (
            scheduled_epoch := module.schedule[0][C._EPOCH_]
        ):
    ...
    def on_train_batch_end(
        ...
        if C._MOAI_LOSSES_ in outputs and "total" in outputs[C._MOAI_LOSSES_]:
            if losses := toolz.merge(
                ...
                module.log_dict(
                    losses, prog_bar=True, logger=False, on_step=True, on_epoch=False
                )
        ...
        if C._MOAI_METRICS_ in outputs:
            if flattened_metrics := outputs[C._MOAI_METRICS_].flatten(separator="/"):
                ...
                module.log_dict(
                    metrics, prog_bar=False, logger=True, on_step=True, on_epoch=False
                )
    ...
    def on_test_epoch_end(
        ...
        for dataset in log_metrics.keys():
            ds = tablib.Dataset(
                [v for v in log_metrics[dataset].values()],
                headers=toolz.keymap(
                    lambda k: k.split("/")[2], dict(log_metrics[dataset])
                ).keys(),
            )
            with open(
                os.path.join(os.getcwd(), f"{logger.name}_{dataset}_test_average.csv"),
                "a",
                newline="",
            ) as f:
    ...
    def on_validation_batch_end(
        ...
        if not hasattr(module.data, "val"):  # NOTE: is this necessary?
            log.warning(
                "No validation data found, validation batch end will be omitted."
            )
            return
        ...
        if C._MOAI_METRICS_ in outputs:
            ...
            dataset_val_metrics = toolz.valmap(
                ...
                toolz.groupby(  # group by last key, i.e. dataset
                    lambda k: k[0].split("/")[-1], scalar_metrics.items()
                ),
            )
            if scalar_metrics:
                module.scalar_metrics[
                    datasets[dataloader_idx]
                ].append(  # format: dataset/named_metric
                    toolz.valmap(
            ...
            non_scalar_metrics = toolz.keymap(
                ...
                toolz.valfilter(lambda x: torch.numel(x) > 1, flattened_metrics),
            )
    ...
    def on_validation_epoch_end(
        ...
        if module.scalar_metrics:
            for i, (dataset, metrics) in enumerate(module.scalar_metrics.items()):
                ...
                for metric_name in keys:
                    ...
                    all_scalar_metrics[dataset][metric_name] = metric_module.compute(
                        np.stack([d[metric_name] for d in metrics])
                    )
                    log_all_metrics[f"{metric_name}/{dataset}"] = float(
        ...
        if module.non_scalar_metrics:
            for i, (dataset, metrics) in enumerate(module.non_scalar_metrics.items()):
                ...
                for metric_name in metric_names:
                    ...
                    inputs = list(
                        toolz.keyfilter(
                            lambda k: k.startswith(metric_name), metrics[0]
                        ).keys()
                    )
                    args = []
                ...
                all_metrics[dataset] = (
                    toolz.merge(
                        all_scalar_metrics[dataset], all_non_scalar_metrics[dataset]
                    )
        ...
        for dataset in all_metrics.keys():
            ds = (
                tablib.Dataset(headers=all_metrics[dataset].keys())
            ...
            with open(
                os.path.join(
                    os.getcwd(), f"{module.logger.name}_{dataset}_val_average.csv"
                ),
                "a",
                newline="",
            ) as f:
