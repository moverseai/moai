...
class Pano3D(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.loaders = [
            functools.partial(Pano3D.__LOADERS__[type], max_depth=max_depth)
            for type in types
        ]
    ...
    def _get_filenames(
        self, folder: str, split: typing.Union[str, pathlib.Path]
    ) -> typing.Sequence[typing.Union[str, pathlib.Path]]:
        if not os.path.exists(split):
            log.error((f"Pano3D split file ({split}) does not exist. "))
            sys.exit(2)
