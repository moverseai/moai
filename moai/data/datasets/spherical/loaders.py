...
def load_depth(filename: str, max_depth: float = 8.0, **kwargs) -> torch.Tensor:
    ...
    depth = torch.from_numpy(cv2.imread(depth_filename, cv2.IMREAD_ANYDEPTH)).unsqueeze(
        0
    )
...
def load_normal(filename: str, **kwargs) -> torch.Tensor:
    ...
    return {
        key: torch.from_numpy(
            cv2.imread(
                normal_filename, cv2.IMREAD_ANYDEPTH | cv2.IMREAD_ANYCOLOR
            ).transpose(2, 0, 1)
        )
    }
...
def load_structure(filename: str, **kwargs) -> torch.Tensor:
    ...
    return {
        key: torch.from_numpy(
            cv2.imread(filename.replace("emission", "layout").replace(".png", ".exr"))
        ).long()
    }
...
def load_layout(filename: str, **kwargs) -> typing.Mapping[str, torch.Tensor]:
    ...
    ret["top_layout"] = (128.0 - ret["top_layout"]) / 128.0
    ret["bottom_layout"] = torch.from_numpy(layout[1, ...])
    ret["bottom_weights"] = (ret["bottom_layout"] > 0).float()
    ret["bottom_layout"] = (ret["bottom_layout"] - 128.0) / 128.0
