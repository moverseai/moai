...
def load_color_image(
    ...
    img = torch.from_numpy(cv2.imread(filename).transpose(2, 0, 1)).flip(dims=[0])
...
def load_gray_image(
    ...
    img = torch.from_numpy(cv2.imread(filename, cv2.IMREAD_GRAYSCALE)).unsqueeze(0)
...
def load_mask_image(
    ...
    img = torch.from_numpy(cv2.imread(filename).transpose(2, 0, 1))
