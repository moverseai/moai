...
class HanCo(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.loaders = [
            functools.partial(HanCo.__LOADERS__[type]) for type in data_types
        ]

    def _get_filenames(
        self, root: str, splits: typing.List[str]
    ) -> typing.Dict[str, typing.List[str]]:
        if not os.path.exists(root):
            logger.error((f"Invalid HanCo root folder ({root}), "))
            sys.exit(2)
        ...
        for filename in rgb_base:
            ...
            for viewpoint in range(8):
                new_name = filename.replace(
                    last, os.path.join("cam" + str(viewpoint), last)
                )
                rgb_filenames.append(new_name)

        shape_filenames = [
            filename.replace("rgb", "shape") for filename in rgb_filenames
        ]  # this referes to the per image shape
        shape_filenames = [
            filename.replace("jpg", "json") for filename in shape_filenames
        ]
        mask_fg_filenames = [
            filename.replace("rgb", "mask_fg") for filename in rgb_filenames
        ]
        mask_hand_filenames = [
            filename.replace("rgb", "mask_hand") for filename in rgb_filenames
        ]
        xyz_filenames = [
            filename.replace("shape", "xyz") for filename in base_filenames
        ]
        calib_filenames = [
            filename.replace("shape", "calib") for filename in base_filenames
        ]
