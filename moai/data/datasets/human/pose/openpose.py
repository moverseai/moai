...
class OpenPoseInferred(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int) -> typing.Dict[str, torch.Tensor]:
        ...
        keypoint_filename = os.path.join(
            os.path.dirname(folder), base_dir, f"{name}_keypoints.json"
        )
        data = self._read_keypoints(
            keypoint_filename,
            self.params.load_hands,
            self.params.load_face,
            self.params.load_face_contour,
        )
        keypoints = data["keypoints"]
        if self.params.single_person_only:

            def _get_area(kpts: torch.Tensor) -> float:
                ...
                keypoints = kpts[..., :2]
                confidence = kpts[..., 2:3]
                inds = confidence[:15, ...][..., 0] > 0.4
                if inds.sum() == 0:
                    return 0.0
                min_x = keypoints[:15, ...][inds, 0].min()
                min_y = keypoints[:15, ...][inds, 1].min()
                max_x = keypoints[:15, ...][inds, 0].max()
                max_y = keypoints[:15, ...][inds, 1].max()
                return (max_x - min_x) * (max_y - min_y)  # * keypoints[..., 2].sum()

            keypoints = [max(keypoints, key=_get_area)] if keypoints else []
        keypoints = torch.stack(keypoints, dim=0).squeeze()
        keypoints[..., 2].scatter_(
            dim=0, index=torch.Tensor(self.invalid_joints).long(), value=0.0
        )[:, np.newaxis]
        kpts = keypoints[..., :2]
    ...
    def _read_keypoints(
        self, filename: str, load_hands=True, load_face=True, load_face_contour=False
        ...
        for person in data["people"]:
            ...
            gender_pd.append(
                person.get("gender_pd", None) or person.get("gender", None)
            )
...
class OpenPoseKeypoints(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.types = (
            OpenPoseKeypoints.__ALL_TYPES__
            if types == "all" or types == "**"
            else types
        )
    ...
    def _extract_data(self, data: dict) -> typing.Dict[str, torch.Tensor]:
        ...
        if not self.single_person:
            ...
            person = (
                max(persons, key=_get_area)
                if persons
                else {
                    "body": {
                        "keypoints": torch.zeros(size=(25, 2), dtype=float),
                        "confidence": torch.zeros(size=(25, 1), dtype=float),
                    },
                    "all": {
                        "keypoints": torch.zeros(size=(135, 2), dtype=float),
                        "confidence": torch.zeros(size=(135, 1), dtype=float),
                    },
                }
            )
            return person
...
class OpenPoseMultiviewKeypoints(OpenPoseKeypoints):
    ...
    def __getitem__(self, index: int) -> typing.Dict[str, torch.Tensor]:
        ...
        for k, v in self.filenames.items():
            data["poses2d"][k] = super()._extract_data(
                super()._load_keypoints(v[index])
            )
        return data
