...
class HBW(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int) -> typing.Dict[str, torch.Tensor]:
        ...
        kpts_filename = image_filename.replace('images', 'keypoints').replace('.png', '.json')
        data = {
        ...
        if self.split == 'val':
            id = os.path.basename(os.path.dirname(os.path.dirname(image_filename))).split('_')[0]
            mesh = trimesh.load(os.path.join(self.root, 'smplx', 'val', f"{id}.obj"), process=False)
            data['smplx'] = {
                'mesh': {
