...
try:
    ...
    log.warning(
        "Could not load the `orjson` package which will improve loading speeds, please consider `pip install orjson`."
    )
...
class Fit3D(torch.utils.data.Dataset):
    def __init__(
        ...
        is_all_actions = isinstance(actions, str) and (
            "all" == actions or "**" == actions
        )
        actions = (
            list(
                map(
        ...
        self.device = (
            f"cuda:{device[0]}"
            if isinstance(device, typing.Sequence) and device[0] >= 0
            else "cpu"
        )
        self.data = {
            ...
            for action_fn in tqdm.tqdm(
                ...
                for k, v in data.items():
                    self.data[k] = torch.cat(
                        [
                            self.data[k],
                            torch.from_numpy(np.array(v, dtype=np.float32).squeeze())[
                                ::downsample_factor, ...
                            ].flatten(1),
                        ]
                    )
