...
class EHF(torch.utils.data.Dataset):
    def __init__(
        ...
        self.camera_translation = torch.Tensor(
            [-0.03609917, 0.43416458, 2.37101226]
        ).float()
        self.camera_rotation = torch.Tensor(
            [-2.98747896, 0.01172457, -0.05704687]
        ).float()
    ...
    def __getitem__(self, index: int) -> typing.Dict[str, torch.Tensor]:
        ...
        keypoints, confidence = self._read_keypoints(
            os.path.join(self.root, "keypoints", f"{base}_keypoints.json")
        )
        data = {
            "color": image,
            "keypoints": keypoints,
            ...
            "vertices": torch.from_numpy(
                np.dot(smplx.vertices, self.extrinsics[:3, :3].numpy().T)
                + self.camera_translation.numpy()
            ).float(),
