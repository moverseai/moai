...
class Custom(torch.utils.data.Dataset):
    def __init__(
        ...
        self.inputs = [
            (
                _create_accessor(k)
                if isinstance(k, str)
                else [_create_accessor(kk) for kk in k]
            )
            for k in self.inputs
        ]
        self.extra = [
            {k: _create_accessor(v) for k, v in (e or {}).items()} for e in self.extra
        ]
        self.augmentations = []
    ...
    def __getitem__(self, index: int) -> typing.Dict[str, torch.Tensor]:
        ...
        for i, o, k, e in zip(self.inputs, self.outputs, self.keys, self.extra):
            if isinstance(k, str):
                data = {
                    k: i(item).numpy() if isinstance(i(item), torch.Tensor) else i(item)
                }  # numpy conversion overhead is insignificant
                for ek, ev in e.items():
                    data[ek] = (
                        ev(item).numpy()
                        if isinstance(ev(item), torch.Tensor)
                        else ev(item)
                    )
                augmented = self.composition(**data)
                if augmented[k] is not None:
                    item[o] = (
                        torch.from_numpy(augmented[k])
                        if isinstance(augmented[k], np.ndarray)
                        else augmented[k]
                    )
                ...
                data = {
                    kk: (
                        i[j](item).numpy()
                        if isinstance(i[j](item), torch.Tensor)
                        else i[j](item)
                    )
                ...
                for kk, oo in zip(k, o):
                    if augmented[kk] is not None:
                        ...
                        item[oo] = (
                            ...
                            if isinstance(augmented[kk], np.ndarray)
                            else augmented[kk]
                        )
