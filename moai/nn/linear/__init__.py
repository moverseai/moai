...
def _update_linear_op(name: str, type: typing.Type):
    if name not in __LINEAR_FACTORY__.keys():
        ...
        log.error(
            f"Trying to add an already existing key {name} in the linear operation factory."
        )


def make_linear_op(
    linear_type: str, in_features: int, out_features: int, bias: bool = True, **kwargs
) -> ptnn.Module:
    if linear_type in __LINEAR_FACTORY__.keys():
        ...
        return miu.instantiate(
            __LINEAR_FACTORY__[linear_type],
            **{
                **locals(),
                **kwargs,
                **other_kwargs,
...
def _update_linear_block(name: str, type: typing.Type):
    if name not in __LINEAR_BLOCK_FACTORY__.keys():
        ...
        log.error(
            f"Trying to add an already existing key {name} in the linear block factory."
        )
...
def make_linear_block(
    ...
    **kwargs,
    ...
    if block_type in __LINEAR_BLOCK_FACTORY__.keys():
        return miu.instantiate(
            __LINEAR_BLOCK_FACTORY__[block_type],
            **{
                **locals(),
...
make_linear_relu_bn = functools.partial(
    make_linear_block,
    block_type="linear",
    linear_type="linear",
...
make_linear_lrelu_bn = functools.partial(
    make_linear_block,
    block_type="linear",
    linear_type="linear",
...
make_linear_lrelu = functools.partial(
    make_linear_block,
    block_type="linear",
    linear_type="linear",
...
make_linear_elu = functools.partial(
    make_linear_block,
    block_type="linear",
    linear_type="linear",
    activation_type="elu",
...
make_linear_sigmoid = functools.partial(
    make_linear_block,
    block_type="linear",
    linear_type="linear",
...
make_linear_tanh = functools.partial(
    make_linear_block,
    block_type="linear",
    linear_type="linear",
