...
class SkeletonConvolution(torch.nn.Module):
    def __init__(self, 
        ...
        if in_channels % self.joint_num != 0 or out_channels % self.joint_num != 0:
            log.error("[SkeletonConvolution] Input and output channels should be a multiple of the number of joints.")            
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        return torch.nn.functional.conv1d(
            torch.nn.functional.pad(input, 
                self._padding_repeated_twice, mode=self.padding_mode
            ),
    ...
    def __repr__(self):
        return 'SkeletonConv(in_channels_per_armature={}, out_channels_per_armature={}, kernel_size={}, ' \
                           'joint_num={}, stride={}, padding={}, bias={})'.format(
            self.in_channels_per_joint, self.out_channels_per_joint, 
            self.kernel_size, self.joint_num, self.stride, self.padding, self.bias
        )
