    ...
    **kwargs,
    ...
    if convolution_type in __CONV_FACTORY__.keys():
        return miu.instantiate(
            __CONV_FACTORY__[convolution_type], **{**locals(), **kwargs}
        )
    else:
...
make_conv_1x1 = functools.partial(
    make_conv_op,  # NOTE: only pass conv_type and inout features
    kernel_size=1,
    stride=1,
    dilation=1,
    padding=0,
...
make_conv_3x3 = functools.partial(
    make_conv_op,  # NOTE: only pass conv_type and inout features
    kernel_size=3,
    stride=1,
    dilation=1,
...
def _update_conv_block(name: str, type: typing.Type):
    if name not in __CONV_BLOCK_FACTORY__.keys():
        ...
        log.error(
            f"Trying to add an already existing key {name} in the convolution block factory."
        )
...
def make_conv_block(
    ...
    **kwargs,
    ...
    if block_type in __CONV_BLOCK_FACTORY__.keys():
        return miu.instantiate(
            __CONV_BLOCK_FACTORY__[block_type],
            **{
                **locals(),
