    ...
    def __init__(
        ...
        self.decoders = torch.nn.ModuleDict(
            OrderedDict(
                (k, hyu.instantiate(d))
                for k, d in sorted(
                    toolz.dissoc(modules, "encoder").items(), key=lambda kvp: kvp[0]
                )
            )
        )
        self.enc_fwds = []
        ...
        enc_in = list(
            zip(*[mirtp.force_list(configuration.encoder[prop]) for prop in params])
        )
        enc_out = mirtp.split_as(
            mirtp.resolve_io_config(configuration.encoder["out"]), enc_in
        )
        ...
        for name, decoder in self.decoders.items():
            ...
            dec_out = mirtp.split_as(
                mirtp.resolve_io_config(configuration[name]["out"]), dec_in
            )
            ...
            self.dec_fwds[name] = []
            for keys in dec_in:
                self.dec_fwds[name].append(
                    lambda td, tk=keys, args=params.keys(), dec=decoder, filler=next(
                        get_dec_filler
                    ): filler(
                        td,
                        dec(
