...
class UNet(minet.FeedForward):
    ...
    def __init__(
        ...
        self.gate = (
            hyu.instantiate(gate, bottleneck.features) if gate else torch.nn.Identity()
        )
        # Decoder
        ...
        out_features = (
            out_features
            if isinstance(out_features, typing.Sequence)
            else [out_features]
        )
        if len(out_features) > 1 and len(out_features) != len(configuration.output[0]):
            if configuration.concat_preds:
                log.error(
                    f"Missmatch between UNet's output names ({configuration.output}) and features ({out_features}). UNet is set to concat predictions, please update the features/names as it will crash otherwise."
                )
                exit(-1)
            else:
                log.warning(
                    f"Missmatch between UNet's output names ({configuration.output}) and features ({out_features})."
                )
        for idx in range(len(block_features), len(out_features), -1):
            out_features.insert(
                len(block_features) - idx, 0
            )  # zero means no prediction
        last_pred_features = 0
        for f, b, sp, o in zip(
            reversed(block_features), up_block_convs, skip_params, out_features
        ):  # TODO: can modify upscale block
            self.up.add_module(
                f"up{f}",
                torch.nn.Sequential(
                    mius.make_upsample(
            ...
            self.skip.add_module(
                f"skip{f}",
                (
                    hyu.instantiate(skip, f, **sp)
                    if skip is not None
                    else UNet.DefaultSkip()
                ),
            )
            dec_block = torch.nn.Sequential()
            skip_features = int(f * expansion) + (
                last_pred_features if configuration.concat_preds else 0
            )
            for i in range(b):
                dec_block.add_module(
                    f"dec{f}_{i}",
                    mic.make_conv_block(
                        block_type="conv2d",
                        convolution_type=convolution.type,
