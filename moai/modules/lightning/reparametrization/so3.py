...
class S2S2Mean(torch.nn.Module):
    ...
    def __init__(self, input_dims):
        super().__init__()
        self.map = torch.nn.Linear(input_dims, 6)
...
class SO3Prior(torch.nn.Module):
    ...
    def forward(self, x_enc, n=1):
        self.mu_lie = self.mean_module(x_enc)
        self.v, sigma = self.reparameterize(x_enc, n)
        
        self.z = self.nsample(n = n)
        return self.v.squeeze(), sigma, self.z.reshape(self.z.shape[0],-1)

    def nsample(self, n=1):
        if self.return_means:
            return self.mu_lie.expand(n, *[-1]*len(self.mu_lie.shape))
        v_lie = rodrigues(self.v.squeeze())
