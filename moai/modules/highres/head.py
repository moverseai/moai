...
class TopBranch(torch.nn.Module):
    def __init__(
        ...
        self.conv = mic.make_conv_block(
            ...
            convolution_params={"kernel_size": kernel_size, "padding": padding},
            activation_params={"inplace": inplace},
        )
        self.sigmoid = torch.nn.Sigmoid()

    def forward(
        self,
        all_branches: typing.Union[
...
class AllBranches(torch.nn.Module):
    def __init__(
        ...
        self.projection = mic.make_conv_block(
            ...
            activation_params={"inplace": projection.inplace},
        )
        self.prediction = mic.make_conv_block(
            ...
            activation_params={"inplace": prediction.inplace},
        )
        self.upsample = functools.partial(
            torch.nn.functional.interpolate, mode=upsample.mode
        )
...
class Higher(torch.nn.Module):
    def __init__(
        ...
        start_features_ = lambda i: (
            start_features if i == 0 else deconvolution.deconv_out_features
        )
                ...
                torch.nn.Sequential(
                    *[
                        midec.make_deconv_block(
                            ...
                            in_features=(
                                (start_features_(i) + out_features)
    ...
    def forward(
        ...
        if not isTraining:
            final_outputs.append(torch.mean(torch.stack(heatmaps_avg), dim=0))
