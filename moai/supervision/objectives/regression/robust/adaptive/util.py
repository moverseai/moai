...
def log_safe(
    ...
    return torch.log(torch.min(torch.as_tensor(x), torch.tensor(max).to(x)))
...
def log1p_safe(
    ...
    return torch.log1p(torch.min(torch.as_tensor(x), torch.tensor(max).to(x)))
...
def exp_safe(
    ...
    return torch.exp(torch.min(torch.as_tensor(x), torch.tensor(max).to(x)))
...
def expm1_safe(
    ...
    return torch.expm1(torch.min(torch.as_tensor(x), torch.tensor(max).to(x)))
...
def inverse_softplus(
    ...
    x = torch.as_tensor(x)
    return torch.where(x > max, x, torch.log(torch.expm1(x)))


def logit(x: typing.Union[float, torch.Tensor, np.ndarray]):
...
def affine_softplus(
    ...
    shift = inverse_softplus(torch.tensor(1.0))
