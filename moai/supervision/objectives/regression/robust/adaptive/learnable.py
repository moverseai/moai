...
from moai.supervision.objectives.regression.robust.adaptive.distribution import (
    Distribution,
)
from moai.utils.arguments.common import assert_non_negative, assert_numeric
...
class Barron(Distribution):
    def __init__(
        ...
        if alpha_range[0] == alpha_range[1]:  # constant init alpha
            ...
            self.register_buffer(
                "fixed_alpha",
                torch.tensor(alpha_range[0])[np.newaxis, np.newaxis].repeat(1, count),
            )
        ...
        if scale_low == scale_init:  # fix `scale` to be a constant.
            ...
            self.register_buffer(
                "fixed_scale",
                torch.tensor(scale_init)[np.newaxis, np.newaxis]
                .float()
                .repeat(1, count),
            )
    ...
    def forward(self, gt: torch.Tensor, pred: torch.Tensor) -> torch.Tensor:
        return super().forward(
            pred.flatten(1) - gt.flatten(1), self.get_alpha(), self.get_scale()
        )
...
class Student(torch.nn.Module):
    def __init__(
        ...
        self.register_parameter(
            "log_df",
            torch.nn.Parameter(torch.zeros((1, count)).float(), requires_grad=True),
        )
    ...
    def lossfun(
        ...
        return util.students_t_nll(
            pred.flatten(1) - gt.flatten(1), self.df(), self.scale()
        )
